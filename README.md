...
它不是一个宣传页。七件事都是为了让线下那两个小时更好用：

| | |
| --- | --- |
| **报名** | 一张 30 秒填完的表。人数直接决定跟店里怎么订位子 |
| **成员墙** `/members` | 一人一张卡：在做什么、想找什么、能帮什么。**来之前先知道该找谁聊**，比到现场花半小时试探快得多。报名时勾一下就能上墙 |
| **码头** `/wharf` | 大家报名时写的那句「最想问什么」都挂在这儿，按场次排。**看到你答得上来的，周四找那个人聊十分钟就完了**——不用到现场再从头认人。没有新的输入框：那句话报名表本来就在收 |
| **场次** `/sessions` | 每一场一行：那天的线稿插画、那天的照片、那天挂在码头上的问题、那天来的人。**在这一页之前，一场活动不是这个站上的一样东西**，是三个页面各知道一部分 |
| **作品** `/works` | 成员墙上所有的产品，按阶段筛选。顺序跟墙一样按最近来过，**不按做到哪一步**——按阶段排会变成排行榜 |
...
scripts/          图片管线、图标生成、端到端脚本
                  session-poster.mjs 一条命令画一场的手绘海报
...
**给一场活动画海报**（`gpt-image-2` 图生图）：

```bash
OPENAI_API_KEY=... node scripts/session-poster.mjs 05   # 或 all
```

**它画的是那天真实的房间**——那一场自己的照片当参考图喂进去（`/v1/images/edits`），
所以窗户、桌子的摆法、视角都是对的。场景说明也不用自己写，它读 `content.ts` 里那一场的 `note`。

★ **调色板用的是站上那一套**（近黑 + 电光青柠 + 青色火花），不是通用赛博朋克的粉紫。
这是为什么这几张画能直接躺在深色页面上，而不用像画在白纸上那样被框起来。

两条硬规矩写在 prompt 里：图里**绝不出现文字**（站上每一句都是真文字，要出中/繁/英三份），
以及 🔴 **不画脸**。第二条在改成图生图之后变得更要紧——原照片里认得出的脸是用贴纸遮掉的，
而让模型照着重画，很容易把贴纸的位置重新画成一张脸，等于把遮脸这条规矩绕过去。
**每次生成之后都要放大逐张核一遍。**
