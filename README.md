...
| **场次** `/sessions` | 每一场一行：那天的手绘海报、那天的照片、那天挂在码头上的问题、那天来的人。**在这一页之前，一场活动不是这个站上的一样东西**，是三个页面各知道一部分 |
...
**给一场活动画海报**（吉卜力风格，`gpt-image-2`）：

```bash
OPENAI_API_KEY=... node scripts/session-poster.mjs 05   # 或 all
```

场景不用自己写——它读 `content.ts` 里那一场的 `note`，也就是当天真实发生了什么。
图里**绝不出现文字**（站上每一句都是真文字，要出中/繁/英三份），也**没有认得出的脸**
（照片的规矩是遮脸，画不该把它悄悄绕过去）。
