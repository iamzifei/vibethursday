...
| **场次** `/sessions` | 每一场一行：那天的彩铅画、那天的照片、那天挂在码头上的问题、那天来的人。**在这一页之前，一场活动不是这个站上的一样东西**，是三个页面各知道一部分 |
...
所以窗户、光线、桌子的摆法都是对的。场景说明也不用自己写，它读 `content.ts` 里那一场的 `note`。
